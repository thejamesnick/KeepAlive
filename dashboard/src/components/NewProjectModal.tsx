    ...
    steps:
        ...
        run: |
          ...
          if [ -n "$SUPABASE_URL" ] && [ -n "$SUPABASE_KEY" ]; then
            ...
            if [ "$HTTP_CODE" -eq 200 ]; then 
                STATUS="ok"
                echo "✅ Supabase Health Check: PASSED ($HTTP_CODE)"
            else
                echo "❌ Supabase Health Check: FAILED ($HTTP_CODE)"
            fi
          fi
          ...
          if [ -n "\${{ secrets.DATABASE_URL }}" ]; then
             if psql "\${{ secrets.DATABASE_URL }}" -c "SELECT 1" > /dev/null 2>&1; then
               ...
               echo "❌ Postgres Connection Check: FAILED"
             fi
          fi
      ...
      - name: Report to KeepAlive Dashboard
        ...
        run: |
          ...
          echo "Response from Dashboard:"
          echo "$RESPONSE"
          
          # Fail if not 200
          if [[ "$RESPONSE" != *"HTTP_CODE:200"* ]]; then
             echo "❌ Dashboard Report: FAILED"
             echo "::error::Failed to report to dashboard"
             exit 1
          else
             echo "✅ Dashboard Report: SUCCESS"
          fi`;
